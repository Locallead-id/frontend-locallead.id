...
const AssessmentCard = ({ assessment, aspectRatio = "portrait", width, height, progressAssessment, className, ...props }: AssessmentCardProps) => {
  const [progress, setProgress] = useState(0);

  useEffect(() => {
    const timer = setTimeout(() => setProgress(Number(progressAssessment)), 500);
