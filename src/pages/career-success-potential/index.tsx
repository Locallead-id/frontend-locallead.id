...
const CareerSuccessPotentialPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get("http://localhost:3000/api/assessments/1/result", {
          headers: {
  ...
  const nonOrganisationalSuccess = score[0]?.non_organisational_success * 10 || 0;
    ...
    <div className="p-4 md:p-8 overflow-auto h-full">
      ...
      <div>
        <div className="flex items-center justify-center mt-8">
          <h1 className="text-lg font-semibold">Career Success Potential Score</h1>
        </div>
        ...
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2 mt-8">
          <Card className="flex flex-col items-center p-4">
            ...
            <p className="flex flex-1 text-justify items-center justify-center text-sm mt-4 px-8">{data.summary || "No summary provided."}</p>
          </Card>
          <Card className="flex flex-col items-center p-4">
            <img src="https://raw.githubusercontent.com/RezaConz/Mocci/7498a86eea932e121f572d2a711c298915f8bbe9/Summary.svg" alt="Summary" />
            <h1 className="text-lg font-medium">Ready to retake the test?</h1>
            <p className="flex text-center items-center justify-center text-sm mt-4 px-8">You’re allowed to retake the test after a minimum of 3 months</p>
