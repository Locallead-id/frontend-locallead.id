...
const leadershipProfile = [
  {
    ...
    description: "Menggambarkan sejauh mana pemimpin mampu mentolerir ketidakpastian yang terjadi",
    score: 94.67,
  },
  {
    name: "Tolerance of Uncertainty",
      ...
      <div className="grid md:grid-cols-2 gap-8">
        <div className="flex flex-col md:ms-2 lg:ms-4">
          ...
          <div className="flex flex-1 items-center justify-center">
            <ChartContainer config={chartConfig} className="mx-auto md:h-[200px] lg:h-[300px] w-auto mt-4">
              <RadarChart data={chartData}>
                <ChartTooltip cursor={false} content={<ChartTooltipContent />} />
                <PolarAngleAxis dataKey="skill" />
                <PolarGrid />
                <Radar dataKey="score" fill="var(--color-desktop)" fillOpacity={0.6} />
              </RadarChart>
            </ChartContainer>
          </div>
        </div>
        <div className="md:ms-2 lg:ms-4 lg:me-12">
          ...
          <h3 className="font-bold text-2xl mb-6 mt-4">{userProfile?.profile?.fullName}</h3>
          ...
          <div className="flex flex-grow bg-slate-200 dark:bg-slate-800 p-4 rounded-lg">
            <p className="text-base font-medium dark:text-[#36DBFF] text-cyan-500">Tolerance of Uncertainty: 94.67%</p>
          </div>
          <p className="mt-6">Menggambarkan sejauh mana pemimpin mampu mentolerir ketidakpastian yang terjadi</p>
            ...
            <div className="grid md:grid-cols-2 grid-cols-1">
              ...
              <div className="flex items-center w-full my-6">
                <div className="w-full mx-12 relative flex items-center flex-1 h-2 bg-slate-200 dark:bg-gray-600 rounded-full">
                  <div className="absolute left-0  h-2 bg-cyan-500 rounded-full" style={{ width: `${profile.score + 2}%` }} />
                  <div className="absolute  w-4 h-4 bg-cyan-300 rounded-full" style={{ left: `${profile.score}%` }} />
                </div>
              </div>
            </div>
          ))}
